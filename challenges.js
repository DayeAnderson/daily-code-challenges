...
// create an array suπmNumbers that takes an array and return the sumNumbers
...
function range(x, y) {
  const z = []; //create empty array
  if(x > y) { //edge case code
    return "First argument must be less than second"
  } 
  for (let i = x; i < y; i++){ // go through the numbers that were given(loop)
    z.push(i)
  }
  return z
}
...
function charCount(str) {
  //return object that represents count of each char
  ...
  return count
  //have keys that rep char with its value set to how many times it appears
}
...
// create a function callled formatWithPadding with 3 arguments
function formatWithPadding(num, str, pad) {
  if (num.length >= pad) {
    return num
  } else {
    return num.toString().padStart(pad, str)
  }
}
...
//   let newStr = str.toLoweCase().split(' ').join().split('')
...
function mumble(str) {
  //loop over string to get position
  //characters repeated based on position
  //add hyphen between characters
  //character doesnt have hypen before
  let result = str[0]
  for(let i = 1; i < str.length; i++){
    result += '-' + str[i].repeat(i + 1) // repeated characters based on position
  }
  return result
}
...
// 194 power of 4 base 256
// 156 power of base 2base 256
// 99 power of 1 base 256
// 15 power of 0 base 256
// result = 194*(256^3) + 156 *(256^2) + 99 *(256^1) + 15 *(256^0)
// powerArray = [3,2,1,0]
// loop through String
// set sum = 0
// sum = str[i]*(256**powerArray[i])
...
function gridTrip(pos, str) {
  const regex = /[UDLR]\d+/g;
  moves = str.match(regex)
  moves.forEach((element, index, array) => {
    if (element.charAt(0) === "D") {
      num = parseInt(element.slice(1))
      pos[0] = pos[0] - num
...
function totalTaskTime(arr, threads){
  let time = 0
  if (arr.length === 0){
    return time
  } else if (threads === 1 ){
    time = arr.reduce(function (accumulator, current) {
    ...
    arr.forEach((element)=> {
      threadArr.sort((a,b) => a-b)
      threadArr[0] = threadArr[0] + element
    })
  threadArr.sort((a,b) => a-b)
  time = threadArr[threads-1]
  }
  return time
}
